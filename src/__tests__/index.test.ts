...
describe('JiraAioServer', () => {
  ...
  beforeEach(() => {
    ...
    process.env = {
      AIO_URL: 'https://jira-aio.example.com',
      AIO_TOKEN: 'test-token',
      AIO_DEFAULT_PROJECT: 'DEFAULT'
    };
  ...
  describe('Configuration', () => {
    test('should throw if AIO_URL is not defined', () => {
      // Arrange
      process.env.AIO_URL = '';
    ...
    test('should throw if neither token nor credentials are provided', () => {
      ...
      expect(() => {
        require('../index');
      }).toThrow('Either AIO_TOKEN or AIO_USERNAME/PASSWORD is required');
    });

    test('should configure axios with token and read default project', () => {
      // Arrange
      const expectedConfig = {
        baseURL: 'https://jira-aio.example.com/rest/api/1.0',
    ...
    test('should throw error when no project is provided or defaulted', async () => {
      ...
      await expect(mockHandleRequest('create_pull_request', input))
        .rejects.toThrow(new McpError(
          ...
          'Project must be provided either as a parameter or through JIRA_DEFAULT_PROJECT environment variable'
        ));
    });
